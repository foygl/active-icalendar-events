...
describe ActiveIcalendarEvents do
  def run_active_events_test(ical_file_path, datetime_s, zone_s, expected_active_events)
    ...
    ical_data = File.open(ical_file_path, 'r') do |ical_file|
    ...
    expect(active_events).to match_array(expected_active_events),
                             "expected #{expected_active_events.inspect} for #{datetime}, got #{active_events.inspect}"
  end
  ...
  it 'check weekly, uk timezone over daylight savings, 25 instances' do
    ...
    expected_active_days.each do |day|
      expected_active_times.each do |time|
      ...
      expected_inactive_times.each do |time|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
          'Europe/London',
    ...
    expected_inactive_days.each do |day|
      (expected_active_times + expected_inactive_times).each do |time|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
          'Europe/London',
  ...
  it 'check weekly, uk timezone over daylight savings, 25 instances, with deletions' do
    ...
    (expected_deleted_days + expected_inactive_days).each do |day|
      (expected_active_times + expected_inactive_times).each do |time|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
  ...
  it 'check weekly, uk timezone over daylight savings, 25 instances, with deletions and moves' do
    ...
    expected_moved_days.each do |_, times_to_check|
      times_to_check[:active_times].each do |time|
        run_active_events_test(
          ical_file_path,
          time,
          'Europe/London',
      ...
      times_to_check[:inactive_times].each do |time|
        run_active_events_test(
          ical_file_path,
          time,
          'Europe/London',
  ...
  it 'check daily, uk timezone over daylight savings, from 2022-10-26 to 2022-11-03' do
    ...
    expected_active_days.each do |day|
      expected_active_events_at_times.each do |time, active_events|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
          'Europe/London',
    ...
    expected_inactive_days.each do |day|
      expected_active_events_at_times.each do |time, _|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
          'Europe/London',
    ...
    expected_active_events_at_specific_datetimes.each do |datetime, active_events|
      run_active_events_test(
        ical_file_path,
        datetime,
        'Europe/London',
  ...
  it 'check daily, uk timezone over daylight savings, from 2022-10-26 to 2022-11-03, with deletions' do
    ...
    (expected_inactive_days + expected_deleted_days).each do |day|
      expected_active_events_at_times.each do |time, _|
        run_active_events_test(
          ical_file_path,
          "#{day} #{time}",
